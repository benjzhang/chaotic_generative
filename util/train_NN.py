...
from util.losses import divergence, divergence_mb, gradient_penalty
...
# -------------------------------------------
def train_disc(parameters, phi, P, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer, print_vals=False):
    ...
    #current_loss = -1e+6
    for in_it in range(1, epochs_nn+1): # Loop for training NN discriminator phi*
        ...
        if optimizer == 'sgd':
            dW_norm = sgd(W, b, nu, dW, db, dnu, lr_NN, NN_par, descent=False, calc_dW_norm=print_vals) # update phi(W,b)
        elif optimizer == 'adam':
            if in_it == 1:
                m_W, v_W, m_b, v_b, m_nu, v_nu = [0]*len(W), [0]*len(W), [0]*len(W), [0]*len(W), 0, 0
            dW_norm, m_W, v_W, m_b, v_b, m_nu, v_nu = adam(W, b, nu, dW, db, dnu, m_W, v_W, m_b, v_b, m_nu, v_nu, lr_NN, NN_par, in_it, descent=False, calc_dW_norm=print_vals) # update phi(W,b)
