...
# ----------------------------------------------------------
from util.losses import f_star

@tf.function
def first_variation(phi, P, W, b, NN_par, loss_par, data_par):
    P_label = data_par['P_label']
    g = phi(P, P_label, W, b, NN_par)
    if loss_par['reverse'] == False: # D(P||Q)
        return g
    else: # D(Q||P)
        return -f_star(g, loss_par)
        '''
        if loss_par['f'] == 'KL':
...
def ABM1(x, deltat, f_xt, aux_params):
    ...
    phi = aux_params['phi']
    Q = aux_params['Q']
    lr_NN = aux_params['lr_NN']
    epochs_nn = aux_params['epochs_nn']
    ...
    f_xt.pop(0)
    parameters = train_disc(parameters, phi, x_tilde, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer)
...
def RK4(x, deltat, f_xt, aux_params):
    ...
    parameters, current_loss1, dW_norm1 = train_disc(parameters, phi, y2, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer, print_vals=True)
    f_xt.append( calc_vectorfield(phi, y2, parameters, NN_par, loss_par, data_par) ) # f_xt[1]
    
    y3 = x - deltat/2*f_xt[-1]
    parameters, current_loss2, dW_norm2 = train_disc(parameters, phi, y3, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer, print_vals=True)
    f_xt.append( calc_vectorfield(phi, y3, parameters, NN_par, loss_par, data_par) ) # f_xt[2]
    
    y4 = x - deltat*f_xt[-1]
    parameters, current_loss3, dW_norm3 = train_disc(parameters, phi, y4, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer, print_vals=True)
...
def ode45(x, deltat, f_xt, aux_params, tol=1e-5):
    ...
    parameters = train_disc(parameters, phi, y2, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer)
    f_xt.append( calc_vectorfield(phi, y2, parameters, NN_par, loss_par, data_par) ) # f_xt[1]
    
    y3 = x - deltat*(3/40*f_xt[0] + 9/40*f_xt[1])
    y3_tilde = x - deltat*3/10*f_xt[0]
    parameters = train_disc(parameters, phi, y3, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer)
    f_xt.append( calc_vectorfield(phi, y3, parameters, NN_par, loss_par, data_par) ) # f_xt[2]
    
    y4 = x - deltat*(44/45*f_xt[0] - 56/15*f_xt[1] + 32/9*f_xt[2])
    y4_tilde = x - deltat*4/5*f_xt[0]
    parameters = train_disc(parameters, phi, y4, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer)
    f_xt.append( calc_vectorfield(phi, y4, parameters, NN_par, loss_par, data_par) ) # f_xt[3]
    
    y5 = x - deltat*(19372/6561*f_xt[0] - 25360/2187*f_xt[1] + 64448/6561*f_xt[2] - 212/729*f_xt[3])
    y5_tilde = x - deltat*8/9*f_xt[0]
    parameters = train_disc(parameters, phi, y5, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer)
    ...
    parameters = train_disc(parameters, phi, y6, Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer)
    ...
    parameters = train_disc(parameters, phi, y7 , Q, lr_NN, epochs_nn, loss_par, NN_par, data_par, optimizer)
