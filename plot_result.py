...
# plot X_ : target data, Y_ : initial data in 2D projected plane
    ...
    f = filename.split('.pickle')
    plt.savefig(f[0]+"-output_target.png")
...
def plot_vectorfield_2D(phi, W, b, NN_par, epochs, filename, X_, trajectory, show=False):
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
        tape.watch(xx)
        phi_xx = phi(xx, None, W, b, NN_par)
    dxx = tape.gradient(phi_xx, xx)
    
    plt.quiver(np.reshape(XX, -1), np.reshape(YY,-1), -dxx[:,0], -dxx[:,1])
    plt.plot(X_[:,0], X_[:,1], '.', label="target")
    plt.plot(trajectory[:,0], trajectory[:,1], '.', label="moving particles")
    plt.legend()
    
    plt.tight_layout()
    
    f = filename.split('.pickle')
    plt.savefig(f[0]+"_%depochs-vectorfield.png" % epochs)
    show_or_close_figure(show)
...
# sample_label: N_samples x num_classes one-hot encoded
    ...
    for i in range(num_classes):
        ...
        if len(i_idx) < print_multiplier:
            i_idx = np.arange(int(i*print_multiplier),int((i+1)*print_multiplier)) #np.random.permutation(samples.shape[0])
...
def plot_multiple_losses(loss_type, exp_alias_, filepath, plot_scale='semilogy', fitting_line=False, save_iter = 1, iter_nos = None, epochs=0, ylims=None, physical_time=True, show=False):
# from one file, plot a designated type of loss (loss_type) 
# iter_nos = [t_1, t_2, t_3,...] marks dots on the loss value of chosen epochs
    ...
    if not re.search("/$", filepath):
        filepath = filepath+"/"
    r = re.compile(".*pickle")
    directories = [x for x in os.listdir(filepath) if not (re.search("(png$|^\.)", x))]
    directories.sort()
...
def plot_result(filename, intermediate=False, epochs = 0, iter_nos = None, data = [], show=False):
    if intermediate == True:
        ...
        trajectories, divergences, KE_Ps, FIDs, X_, Y_, X_label, Y_label, dt, save_iter = None, None, None, None, None, None,  None, None, None, None
        filename_ = filename
        epochs = 0
    ...
    if ('MNIST' in filename  or 'CIFAR10' in filename) and ".pickle" in filename:
        ...
        if intermediate == False:
            ...
            if 'all' in filename or 'cond' in filename: # (un)conditional gpa
                try:
                    plot_tiled_images(print_multiplier=5, samples=None, sample_label=Y_label, epochs = -1, filename=filename_, show = show)
                except:
                    plot_tiled_images(print_multiplier=6, samples=None, sample_label=Y_label, epochs = epochs, filename=filename_, show = show)
        else:
            if 'all' in filename or 'cond' in filename: # (un)conditional gpa
                try:
                    ...
                    plot_tiled_images(print_multiplier=6, samples=trajectories[-1], sample_label=Y_label, epochs = epochs, filename=filename_, show = show)
    ...
    ## low dimensional example
        ...
        plot_vectorfield_2D(phi, W, b, NN_par, epochs, filename, X_=X_, trajectory=trajectories[int(epochs/save_iter)-1], show=show)
        plot_trajectories(trajectories=trajectories, dt=dt, X_=X_, Y_=Y_, dataset = dataset, r_param=r_param, vectorfields = vectorfields, proj_axes = [0,1], pick_samples =None, epochs = epochs, iter_nos = iter_nos, physical_time=True, save_iter = save_iter, track_velocity=track_velocity, arrow_scale = 1, iscolor=iscolor, quantile=quantile, exp_alias_ = exp_alias_, x_lim = x_lim, y_lim = y_lim, filename = filename_, show = show)
    ...
    # Multiple experiments in one plot
        ...
        plot_multiple_trajectories(filename_, exp_alias_, proj_axes = [0,1], pick_samples =None, epochs = 0, iter_nos = None, physical_time=True, save_iter = save_iter, track_velocity=False, arrow_scale = 1, iscolor=iscolor, quantile=quantile, x_lim = x_lim, y_lim = y_lim, show = show)
        plot_multiple_losses(loss_type='divergences', exp_alias_ = exp_alias_, filepath = filename_, plot_scale='semilogy', fitting_line=False, save_iter = 1, iter_nos = None, epochs=0, ylims=None, physical_time=True, show = show)
        plot_multiple_losses(loss_type='KE_Ps', exp_alias_ = exp_alias_, filepath = filename_, plot_scale='semilogy', fitting_line=False, save_iter = 1, iter_nos = None, epochs=0, ylims=None, physical_time=True, show = show)
